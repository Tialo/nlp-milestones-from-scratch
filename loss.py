...
def create_smoothed_vectors(n_rows: int, n_cols: int, confident_inds: torch.Tensor, inds_to_zero: torch.Tensor, smoothing: float = 0.1):
    ...
    assert not any(torch.isin(confident_inds, inds_to_zero)), "confident_inds and inds_to_zero values should not overlap"
    ...
    assert all(abs(smoothed_distribution.sum(axis=1) - 1.0) < 1e-6), "smoothed_distribution does not sum to 1 in each row"
    return smoothed_distribution


class LabelSmoothingLoss(nn.Module):
    def __init__(self, ignore_index: int, smoothing: float = 0.1, use_cross_entropy: bool = True, reduction: str = 'batchmean'):
        super().__init__()
