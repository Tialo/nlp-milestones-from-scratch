...
class Generator:
    ...
    @torch.no_grad
    def _get_top_tokens(self, memory: torch.Tensor, generated: torch.Tensor, n_beams: int) -> tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        logits = self.transformer.decode(memory, generated)  # (batch_size, current_seq_len, vocab_size)
    ...
    def generate(self, src: torch.Tensor, max_tokens: int = 20, n_beams: int = 5) -> torch.Tensor:
        if src.dim() == 1:
            src = src.unsqueeze(0)
        elif src.size(0) != 1:
        ...
        for _ in range(max_tokens):  # current_seq_len
            ...
            batched_tokens_to_process = torch.tensor(tokens_to_process).to(src.device)  # (n_beams - len(finished_beams), current_seq_len)
            top_tokens, proba = self._get_top_tokens(memory, batched_tokens_to_process, n_beams)
            ...
            beams = heapq.nlargest(n_beams, candidate_beams, key=self._get_normalized_score)
